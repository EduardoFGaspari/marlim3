...
A brief discussion of the CFL criterion is warranted at this point. The aim here is not to present a mathematically rigorous derivation, but rather to provide a physical interpretation of this stability restriction. The numerical method employed is the finite-volume method. In the discretization process, the approximate solution may be interpreted as a piecewise-continuous function with compact support; within each control volume, the modeled quantities are represented by continuous solutions, further assumed to be constant throughout that volume. To illustrate this interpretation of a discretized solution, consider the void fraction distribution over a one-dimensional discretized domain:

![discretização da fração de vazio em um domínio unidimensional](../img/disc2.png)

Within volume $i$, the solution is constant, as it is in the neighboring volumes, with value jumps occurring at the interfaces. Under this interpretation, the problem is naturally cast as a sequence of Riemann problems; indeed, as long as the system is regarded as hyperbolic, the evolution between neighboring control volumes may be viewed locally as a succession of Riemann problems over a finite time interval. Within each volume, one may approximately assume constant wave speeds and characteristic directions, determined by the constant state inside that volume, and advance the solution under this simplification for a limited time interval. In fact, an entire class of finite-volume methods is founded on this idea, namely Riemann solvers; see LeVeque (2002). The largest admissible time interval is that required for a wave to traverse a control volume while the propagation speed and the flow variables may still be regarded as constant. Once the wave reaches an interface and encounters the constant state of a neighboring volume, this approximation ceases to be valid, and the wave fluctuations become physically less representative inside the adjacent cell. In other words, in an explicit scheme based on quantities known at time $T_0$, the solution should not be advanced beyond the time required for a wave to cross a control volume. This is, in broad terms, precisely what the CFL condition establishes.
...
Once the profiles of $\alpha$ and $\beta$ have been obtained, the next step is to solve the pressure–velocity coupling problem. As discussed previously, pressure is closely associated with the fastest wave families, which are in turn related to fluid compressibility. These fast waves impose a more restrictive CFL condition; therefore, if time increments larger than those dictated by compressibility are to be used, the equations governing pressure–velocity coupling must be treated implicitly. The equations used to determine the pressure profile and the mixture mass flow rate are:
